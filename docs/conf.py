...
def filter_changelog(
    orig_file: str = "../CHANGELOG.md", new_file: str = "./whats_new.md"
):
    orig_log = pathlib.Path(orig_file).read_text().splitlines()
    changelog = []
    ...
    pathlib.Path(new_file).write_text("\n".join(changelog))
...
def autosummary_map_module_path_to_toplevelobj():
    ...
    return {
        f"{obj['module']}.{obj['name']}": f"{obj['package']}.{obj['name']}"
        for obj in top_objs
    }
...
def autosummary_map_toplevelobj_to_module_path():
    top_objs = create_autocontext()
    return {
        f"{obj['package']}.{obj['name']}": f"{obj['module']}.{obj['name']}"
        for obj in top_objs
    }
