...
def download_from_github(file=None, user=None, repository=None, branch='master', overwrite=False,
                         silent=False):
    ...
    if nba.status_code == 200:
        ...
        if file_exists and not overwrite:
            if not silent:
                printmd(
                    f"File {os.path.abspath(filename)} exists - call `download('{file}', overwrite = True)` in order to download module `{file}` anyway")
        else:
            nba = nba.text
