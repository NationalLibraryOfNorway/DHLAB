...
def ner(text=None, dist=False):
    ...
    if text is not None:
        r = requests.post(
            "https://api.nb.no/ngram/ner", json={"text": text, "dist": dist}
        )
    return r.json()
...
def check_navn(name, limit=2, remove: list = None):
    """Removes all items in navn with frequency below limit,
    and words in all cases as well as all words in list 'remove'.
    """
    remove = (
        (
            "Ja Nei Nå Dem De Deres Unnskyld Ikke Ah Hmm Javel Akkurat " "Jaja Jaha"
        ).split()
        if remove is None
        else remove
    )
    r = {
        x: name[x]
        for x in name
        if name[x] > limit and x.upper() != x and x not in remove
    }
    return r
...
def word_variant(word, form):
    ...
    """
    r = requests.get(
        "https://api.nb.no/ngram/variant_form", params={"word": word, "form": form}
    )
    return r.json()
...
def unigram(
    ...
    r = requests.get(
        "https://api.nb.no/ngram/unigrams",
        params={
            "word": word,
            "ddk": ddk,
            "topic": topic,
...
def book_counts(period=(1800, 2050)):
    r = requests.get(
        "https://api.nb.no/ngram/book_counts",
        params={
            "period0": period[0],
            "period1": period[1],
        },
...
def book_urn(
    ...
    query = {i: values[i] for i in args if values[i] is not None and i != "period"}
    query["year"] = period[0]
...
def refine_book_urn(
    ...
    query = {
        i: values[i]
        for i in args
        if values[i] is not None and i != "period" and i != "urns"
    }
    query["year"] = period[0]
    query["next"] = period[1] - period[0]
    # print(query)
...
def best_book_urn(
    ...
    query = {
        i: values[i]
        for i in args
        if values[i] is not None and i != "period" and i != "word"
    }
    query["year"] = period[0]
    query["next"] = period[1] - period[0]
...
def get_papers(top=5, cutoff=5, name="%", yearfrom=1800, yearto=2020, samplesize=100):
    ...
    for _ in range(first):
        r += requests.get(
            "https://api.nb.no/ngram/avisfreq",
            json={
                "navn": name,
                "top": top,
                "cutoff": cutoff,
...
def compare_word_bags(
    bag_of_words,
    another_bag_of_words,
    first_freq=0,
    another_freq=1,
    top=100,
    first_col=0,
...
def get_corpus(
    top=0, cutoff=0, name="%", corpus="avis", yearfrom=1800, yearto=2020, samplesize=10
):
...
class Cluster:
    ...
    def add_reference(self, number=20):
        ref = get_corpus(
            yearfrom=self.period[0], yearto=self.period[1], samplesize=number
        )
        ref = aggregate(ref)
        ref.columns = ["add_ref"]
    ...
    def search_words(self, words, exponent=1.1):
        ...
        res = (
            df.transpose()[sub]
            .transpose()
            .sort_values(by=df.columns[0], ascending=False)
        )
        return res
...
def make_newspaper_network(
    key, wordbag, titel="%", yearfrom="1980", yearto="1990", limit=500
):
    """NB! Seems not to work at the moment."""
    if isinstance(wordbag, str):
        wordbag = wordbag.split()
    r = requests.post(
        "https://api.nb.no/ngram/avisgraph",
        json={
            "key": key,
            "words": wordbag,
            "yearto": yearto,
...
def make_network_graph(urn, wordbag, cutoff=0):
    ...
    """
    r = requests.post(
        "https://api.nb.no/ngram/graph", json={"urn": urn, "words": wordbag}
    )
    G = nx.Graph()
    G.add_weighted_edges_from(
...
def draw_graph_centrality(
    G, h=15, v=10, fontsize=20, k=0.2, arrows=False, font_color="black", threshold=0.01
):
    ...
    nx.draw_networkx_edges(
        G, pos, alpha=0.7, arrows=arrows, edge_color="lightblue", width=1
    )
...
class Corpus:
    ...
    def __init__(
        ...
        if filename == "":
            ...
            if len(target_corpus_urn) > max_books > 0:
                target_urn = list(numpy.random.choice(target_corpus_urn, max_books))
            else:
    ...
    def conc(self, word, before=8, after=8, size=10, combo=0):
        ...
        return get_urnkonk(
            word, {"urns": urns, "before": before, "after": after, "limit": size}
        )
    ...
    def sort_collocations(self, word, comparison=None, exp=1.0, above=None):
        ...
        try:
            ...
            print(f"Constructing a collocation for {word} " f"with default parameters.")
    ...
    def search_collocations(self, word, words, comparison=None, exp=1.0):
        ...
        return (
            res.transpose()[search_items].transpose().sort_values(by=0, ascending=False)
        )
    ...
    def make_collocation_graph(
        self, target_word, top=15, before=4, after=4, limit=1000, exp=1
    ):
...
def nb_ngram(
    ...
    """
    df = ngram_conv(
        get_ngram(terms, corpus=corpus), smooth=smooth, years=years, mode=mode
    )
...
def ngram_conv(ngrams, smooth=1, years=(1810, 2013), mode="relative") -> pd.DataFrame:
...
def make_graph(words, lang: str = "nob", cutoff: int = 20, leaves: int = 0):
    ...
    result = requests.get(
        "https://www.nb.no/sp_tjenester/beta/ngram_1/galaxies/query", params=params
    )
...
def urn_concordance(urns=None, word=None, size=5, before=None, after=None):
    ...
    dataframe = inspect.currentframe()
    args, _, _, values = inspect.getargvalues(dataframe)
    query = {i: values[i] for i in args if values[i] is not None and i != "word"}
...
def konk(word, urns=None, before=5, after=5):
    ...
    return urn_concordance(
        word=word, urns=sample(urner, min(20, len(urner))), before=before, after=after
    )
...
def concordance(
    ...
    query = {
        i: values[i]
        for i in args
        if values[i] is not None and i != "word" and i != "kind"
    }
...
def frame_sort(dataframe, by=0, ascending=False):
    ...
    if by in dataframe.columns:
        ...
        res = dataframe.sort_values(by=dataframe.columns[col], ascending=ascending)
    else:
        res = dataframe.sort_values(by=dataframe.columns[0], ascending=ascending)
    return res
...
def dewey(dewey_):
    r = requests.get(
        "https://api.nb.no:443/dewey/v1/list",
...
def metadata_xml(URN, kind="marcxml"):
    ...
    r = requests.get(f"https://api.nb.no:443/catalog/v1/metadata/{URN}/{kind}")
    try:
        res = r.text
    # W0702: No exception type(s) specified (bare-except)
    except BaseException:
...
def save_frame_to_excel(df, filename, index=False):
    if os.path.exists(filename):
        print(
            f"Det eksisterer allerede en fil {filename}"
